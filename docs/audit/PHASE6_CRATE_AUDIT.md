...
**Issue:** `emitter.rs:1`, `decl.rs:59` and others show UTF-8 mojibake (`ÃƒÆ'Ã†â€™...`).
**Fix:** Re-save affected files with correct UTF-8 encoding.
...
- **Bugs:** O(n²) clone in postfix chain (line 1337), stale comment (MAX_EXPR_DEPTH=32 not 200)
...
1. ✅ xiom-dbg: Fix compile error (`_child` → `child`)
2. ✅ xiom-pkg: Fix `static mut` → `OnceLock`
3. ✅ xiom-check: Remove `types_compatible` catch-all
4. ✅ xiom-codegen: Default `"i64"` → error
5. ✅ Fix encoding corruption in source files
