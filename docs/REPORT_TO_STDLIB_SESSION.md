# Report to stdlib session â€” UPDATE (2026-08-16, after the BUG 30 regression sweep)
...
| smoke_utf8 | `xiom.convert.utf8` API | ensure `result.len()` on Vec payload â†’ Map.len/Str.len + user-local `result` shadowing | **FIXED** |
| smoke_hash_folder | missing import | `to_int_from_char` not imported (`use xiom.convert.toint;`) | **STDLIB-SIDE â€” your fix** |

`xiom.utf8` (the legacy module name in stdlib/xiom/string/utf8.xi) still
exists and the smoke uses it â€” the convert/utf8.xi API differs as documented.
...
## 904-smoke battery â€” your half of the work
...
1. `smoke_hash_folder.xi` â€” add `use xiom.convert.toint;` (bare
   `to_int_from_char`).
2. P001 parse errors â€” e.g. smoke_stress_rand_shuffle (unbalanced brace),
   check all smoke_stress_rand_* files.
3. `undefined variable` / missing-import families â€” smoke_alloc_basic
   (`ptr` â€” item C on the compiler side: bare prelude names in user
   modules, so ALSO a compiler item), smoke_net_* (drifted APIs),
   smoke_fmt_formatter_* (they call APIs the fmt module doesn't export).
4. Renamed/absent APIs â€” smoke_stress_rand_sample_* etc. align to the
